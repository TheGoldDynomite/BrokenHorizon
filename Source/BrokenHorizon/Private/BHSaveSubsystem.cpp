...
) const
{
    ...
    TMap<FName, const AActor*> FortificationIDs;
    ...
    {
        const ABHFieldFortification* Fortification = *FortificationIt;
        const FName PersistenceID = Fortification->GetPersistenceID();
        ...
        if (PersistenceID.IsNone())
        {
            UE_LOG(
                LogTemp,
                Error,
                TEXT("Field fortification %s has no persistence ID."),
                *Fortification->GetPathName()
            );
            bValid = false;
            continue;
        }
        ...
        if (const AActor* const* Existing =
            FortificationIDs.Find(PersistenceID))
        {
            UE_LOG(
                LogTemp,
                Error,
                TEXT(
                    "Duplicate field fortification persistence ID %s "
                    "on %s and %s."
                ),
                *PersistenceID.ToString(),
                *(*Existing)->GetPathName(),
        ...
        FortificationIDs.Add(PersistenceID, Fortification);
    }
