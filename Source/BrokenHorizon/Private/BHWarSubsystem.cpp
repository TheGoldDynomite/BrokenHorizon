...
constexpr int32 DefaultFortificationCapacityPerSector = 3;
constexpr float FortificationDefenseBonusPerPoint = 0.18f;
constexpr float MaximumFortificationDefenseMultiplier = 1.90f;
...
int32 GetConstructedFortificationCount(
    const UWorld* World,
    FName SectorID
)
{
    if (!World || SectorID.IsNone())
    {
        return 0;
    }
...
)
{
    ...
    Sector.FortificationCapacity = DefaultFortificationCapacityPerSector;
    Sector.ConstructedFortifications = 0;
...
int32 UBHWarSubsystem::GetSectorConstructedFortificationCount(
    FName SectorID
) const
{
    const int32 SectorIndex = FindSectorIndex(SectorID);
    ...
    return GetConstructedFortificationCount(
        GetWorld(),
        SectorID
    );
}

float UBHWarSubsystem::GetSectorFortificationDefenseMultiplier(
    FName SectorID
) const
{
    const int32 SectorIndex = FindSectorIndex(SectorID);
    ...
    const FBHWarSectorState& Sector = SectorStates[SectorIndex];
    const int32 ConstructedCount = GetConstructedFortificationCount(
        GetWorld(),
        SectorID
    );
    const int32 EffectiveCapacity = FMath::Max(
        1,
        FMath::Max(
            Sector.FortificationCapacity,
            DefaultFortificationCapacityPerSector
        )
    );
    const int32 ClampedCount =
        FMath::Clamp(ConstructedCount, 0, EffectiveCapacity);
    const float RawMultiplier =
        1.0f + (static_cast<float>(ClampedCount) * FortificationDefenseBonusPerPoint);

    return FMath::Clamp(
        RawMultiplier,
        1.0f,
        MaximumFortificationDefenseMultiplier
    );
}

int32 UBHWarSubsystem::GetSectorFortificationCapacity(
    FName SectorID
) const
{
    const int32 SectorIndex = FindSectorIndex(SectorID);
    if (!SectorStates.IsValidIndex(SectorIndex))
    {
        return DefaultFortificationCapacityPerSector;
    }
...
bool UBHWarSubsystem::CanPlaceAdditionalFortification(
    FName SectorID,
    int32 AdditionalCount
) const
{
    if (SectorID.IsNone() || AdditionalCount <= 0)
    {
        return false;
    }
    ...
    const int32 Capacity = GetSectorFortificationCapacity(SectorID);
    const int32 ConstructedCount =
        GetSectorConstructedFortificationCount(SectorID);
    return ConstructedCount + AdditionalCount <= Capacity;
}

void UBHWarSubsystem::SynchronizeFortificationStateWithWorld()
{
    ...
    for (FBHWarSectorState& Sector : SectorStates)
    {
        ...
        Sector.ConstructedFortifications = GetConstructedFortificationCount(
            World,
            Sector.SectorID
        );
    }
...
) const
{
    ...
    const float SupplyFactor =
        Sector.Owner == ForceFaction
            ? CalculateCombatSupplyFactor(Sector.Supply)
            : 1.0f;
    const float FortificationMultiplier =
        Sector.Owner == ForceFaction
            ? GetSectorFortificationDefenseMultiplier(
                SectorID
            )
            : 1.0f;
    ...
    ) * SupplyFactor * FortificationMultiplier;
}
...
void UBHWarSubsystem::ReinforceGarrisons()
{
    for (FBHWarSectorState& Sector : SectorStates)
    {
        ...
        const int32 ReinforcedTroops = FMath::Min3(
            MissingTroops,
            AffordableTroops,
            FMath::Min(
                ReinforcementBudget,
        ...
        Sector.Supply -=
            ReinforcedTroops * GarrisonSupplyCostPerUnit;
        ClampSectorState(Sector);
        ...
        UE_LOG(
            ...
            TEXT(
                ...
                "troops=%d garrison=%d/%d supply=%.1f->%.1f "
                "manpower=%d->%d"
            ),
            *Sector.SectorID.ToString(),
            static_cast<int32>(Sector.Owner),
...
)
{
    for (FBHWarSectorState& Sector : SectorStates)
    {
        ...
        {
            const float FortificationDefenseBonus =
                Sector.Owner == EBHWarFaction::Friendly
                    ? GetSectorFortificationDefenseMultiplier(
                        Sector.SectorID
                    )
                    : Sector.Owner == EBHWarFaction::Enemy
                        ...
                        : 1.0f;
            const float FriendlyMultiplier =
                Sector.Owner == EBHWarFaction::Friendly
                    ? (1.0f / FortificationDefenseBonus)
                    : FortificationDefenseBonus;
            const float EnemyMultiplier =
                Sector.Owner == EBHWarFaction::Friendly
                    ? FortificationDefenseBonus
                    : (1.0f / FortificationDefenseBonus);

            const float FriendlyLoss = FMath::Min(
                Sector.FriendlyStrength,
                Sector.EnemyStrength *
                    0.08f *
            ...
            const float EnemyLoss = FMath::Min(
                Sector.EnemyStrength,
                Sector.FriendlyStrength *
                    0.08f *
                    RandomStream.FRandRange(0.8f, 1.2f) *
...
) const
{
    ...
    Sector.FortificationCapacity = FMath::Clamp(
        Sector.FortificationCapacity,
        0,
        6
    );
    Sector.ConstructedFortifications = FMath::Clamp(
        Sector.ConstructedFortifications,
        0,
        Sector.FortificationCapacity
    );
    if (Sector.Owner == EBHWarFaction::Friendly)
