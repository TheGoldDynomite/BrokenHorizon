...
public:
    ...
    float RecoverFortificationSupply(
        FName SectorID,
        float SupplyAmount
    );
    ...
    UFUNCTION(BlueprintCallable, Category = "Persistent War|Intelligence")
    bool ReportEnemyDetained(
        FName SectorID,
        float IntelligenceGain = 12.0f,
        float EnemyStrengthLoss = 2.0f
    );

    UFUNCTION(BlueprintCallable, Category = "Persistent War|Conduct")
    bool ReportSurrenderedEnemyKilled(
        FName SectorID,
        float CivilianSupportLoss = 10.0f,
        float EnemyResponseGain = 15.0f,
        int32 CampaignMeritLoss = 25
    );

    UFUNCTION(BlueprintCallable, Category = "Persistent War|Conduct")
    bool ReportSurrenderedEnemyEscaped(
        FName SectorID,
        float EnemyResponseGain = 5.0f,
        float IntelligenceLoss = 5.0f
    );

    UFUNCTION(BlueprintCallable, Category = "Persistent War|Conduct")
    bool ReportFriendlyFireEscalation(
        FName SectorID,
        float CivilianSupportLoss = 3.0f,
        float EnemyResponseGain = 3.0f,
        int32 CampaignMeritLoss = 10
    );
    ...
    float GetSectorFortificationDefense(FName SectorID) const;

    UFUNCTION(
        BlueprintPure,
        Category = "Persistent War|Simulation"
    )
    void GetSectorFortificationSummary(
        FName SectorID,
    ...
    static float CalculateFortificationCasualtyMultiplier(
...
private:
    ...
    float GetSectorFortificationSupplyBonus(FName SectorID) const;
    int32 GetSectorRallyDeploymentReserve(FName SectorID) const;
    int32 ConsumeSectorRallyDeployments(
        FName SectorID,
