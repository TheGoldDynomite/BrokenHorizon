...
### Cargo-weighted field transport control - 9 August 2026

- Field transport now applies its normalized cargo load fraction to acceleration, braking response, and steering authority through the authorable `LoadedControlMultiplierAtCapacity` setting, defaulting to 0.82 at full cargo. Empty cargo preserves the previous handling response, while partial loads degrade control gradually instead of creating a binary loaded/unloaded state.
- The bounded policy clamps both load and tuning values, does not change transport replication, fuel, hull, cargo, passenger, persistence, or Blueprint-facing state contracts, and composes with the existing weather traction response.
- Evidence: `BHTransportHandling-Build.log`, `BHTransportHandling-Tests.log`, `BHTransportHandling-Smoke.log`, and `BHTransportHandling-FirstLight.log` passed. `BrokenHorizon.Gameplay.Logistics.TransportHandling` completed with `Result={Success}`; First Light recorded 3/12 navigation fallbacks.
- Fresh current-source release evidence: UE5.8 `BuildCookRun` built, cooked, staged, pak/IoStore-packed, compressed, and archived `Builds/CurrentSource-20260809-TransportHandling-Development` with `BUILD SUCCESSFUL` and exit code 0. Its root executable launched First Light; `BHTransportHandling-Packaged-FirstLight.log` contains the First Light map markers, one normal `LogExit: Exiting.` marker, and no fatal, assertion, ensure, unhandled-exception, or package-load failure markers.
- Remaining transport gates are authoritative multiplayer replication/latency, terrain interaction, physical driving and resupply feel, and lower-tier/performance review; these are not inferred from the contract or headless smoke.

### Reload interruption presentation cancellation (2026-08-09)

- Player-visible result: interrupting a tactical or emergency reload through firing input or incoming damage now clears the first-person reload presentation instead of leaving the hands or rifle visually committed to a reload after the authoritative weapon state returns to idle.
- Implementation: `UBHWeaponComponent::InterruptReload` invokes `ABHRifle::CancelReloadPresentation` before the action reset completes. The character clears its first-person action timer and procedural reload offsets; the rifle stops active first-person montages and rifle single-node animation, then restores the normal arms loop.
- Realism value: reload interruption preserves the cost of exposure and the coherent magazine/discarded-round state while making the player-facing result agree with the authoritative combat state.
- Validation: editor build passed; the automation log completed 89 tests including `BrokenHorizon.Gameplay.Combat.ReloadInterruptionPresentation`; startup and First Light source smoke passed with the expected 3/12 authored navigation fallback count; a fresh Win64 BuildCookRun archived to `Builds/CurrentSource-20260809-ReloadInterruption-Development`; packaged First Light completed navigation, four objectives, five route-success markers, and zero fatal/assertion/ensure markers.
- Remaining manual gates: dedicated-client interruption timing, network latency behavior, montage blending, animation/audio handoff, and combat-feel review still require in-editor multiplayer/playtest review.
### Defeated enemy persistence (2026-08-09)
- Player-visible result: a hostile operative killed during an operation remains defeated after save/load instead of respawning as a healthy enemy when its corpse expires.
- Implementation: UBHSaveSubsystem records the stable field-operative ID, sector, and last transform at death; ABHEnemySoldier and UBHHealthComponent restore an inert zero-health state with collision, movement, damage, reload, AI death handling, and corpse presentation applied once.
- Realism value: combat consequences survive persistence boundaries without duplicating death rewards or morale/objective side effects.
- Evidence: editor build passed; focused BrokenHorizon.PersistentWar.EnemyDeathPersistence automation passed; independent startup and First Light source smoke passed; fresh Win64 package and packaged First Light route passed with navigation success, four objective completions, a route completion marker, and zero fatal/assertion/ensure markers.
- Remaining review: aggregate automation still has unrelated failures in DistantEventWeatherMix and SettingsContract from preserved concurrent worktree changes; dedicated multiplayer load timing and in-editor corpse/persistence presentation review remain open.
### Weapon heat and fire-mode persistence contract (2026-08-09)
- Player-visible result: a checkpoint retains a hot or overheated weapon and the selected semi-automatic or automatic mode instead of silently resetting combat handling after load.
- Contract hardening: the focused persistence test verifies SaveGame serialization, owner replication, reflected fire-mode options, heat clamping, heat-dependent spread, current schema coverage, and validity-bit gating for legacy saves.
- Evidence: the editor build passed; BrokenHorizon.PersistentWar.WeaponStatePersistence passed; independent startup and First Light smoke passed with 3/12 authored navigation fallbacks.
- Remaining review: the aggregate suite still reports only the preserved concurrent DistantEventWeatherMix and SettingsContract failures; checkpoint round-trip with a live equipped rifle, dedicated-client replication timing, controller readability, and manual heat/recovery feel remain open.
### Terminal enemy death-state cleanup (2026-08-09)
- Player-visible result: a defeated hostile operative cannot return from a checkpoint as an incapacitated casualty, medical-evacuation target, or surrendering captive.
- Implementation: persistent death restore is server-authority gated, clears incapacitation and casualty timers, clears surrender and custody flags, marks prior conduct outcomes as already reported, disables damage and movement, removes collision, and forces the terminal state to replicate before corpse presentation.
- Evidence: editor build passed; the canonical automation queue passed `BrokenHorizon.PersistentWar.EnemyDeathPersistence`, `BrokenHorizon.PersistentWar.EnemyDeathStateContract`, and `BrokenHorizon.PersistentWar.WeaponStatePersistence`; startup and First Light smoke passed with 3/12 authored navigation fallbacks; a fresh Win64 package archived successfully; packaged First Light completed four objectives and the route with zero fatal/assertion/ensure/unhandled-exception/package-load markers.
- Remaining review: live multiplayer save/load with a surrendered or incapacitated hostile, late-spawn replication timing, corpse presentation, authored NavMesh coverage, and the preserved aggregate `DistantEventWeatherMix` and `SettingsContract` failures remain open.
### Live terminal death cleanup (2026-08-09)
- Player-visible result: when a surrendered or medically marked hostile dies, its corpse no longer remains actionable as a captive or casualty while the death presentation is still visible.
