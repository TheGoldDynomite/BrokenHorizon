...
| GDD-08 World/art/audio | In progress | First Light target, region map, lighting/presentation automation, read-only UE metadata/reference-chain audit for 234 shipping-scope assets including the active rifle dependency, authoritative LOD/collision metadata, replicated AI voice barks, movement acoustics, weather/war bed, semantic UI cues, indoor/outdoor weapon tails, directional near misses, and canonical audio/FX assignment audit | Assign or author 17 missing required and 27 optional audio/FX cues; resolve 2 shipping-referenced named candidates; manually disposition the single-LOD ejected casing and complete material/look, audio, streaming and runtime performance gates |
